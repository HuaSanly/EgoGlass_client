...
def test_repeated_desktop_server_lifecycle_is_private_and_recoverable() -> None:
    ...
    for iteration in range(3):
        ...
        try:
            try:
                urllib.request.urlopen(f"{server.origin}/api/v1/state", timeout=5)
            except urllib.error.HTTPError as error:
                assert error.code == 401
            else:
                raise AssertionError("desktop API accepted a request without its session token")
            ...
            with opener.open(f"{server.origin}/api/v1/state", timeout=5) as response:
                assert b'"mode":"simulation"' in response.read()
        finally:
            server.stop()
