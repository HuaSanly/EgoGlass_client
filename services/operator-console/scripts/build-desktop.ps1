...
$pyinstaller = Join-Path $serviceRoot ".venv\Scripts\pyinstaller.exe"
...
try {
    ...
    $executable = Join-Path $serviceRoot "dist\EgoGlass\EgoGlass.exe"
    if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
        throw "Desktop executable was not created at $executable"
    }

    $noticesSource = Join-Path $serviceRoot "packaging\THIRD_PARTY_NOTICES.txt"
