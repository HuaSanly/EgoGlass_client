...
def test_pyinstaller_spec_resolves_all_packaged_inputs() -> None:
    ...
    spec_path = service_root / "packaging" / "egoglass-console.spec"
    ...
    assert Path(captured["datas"][0][0]).is_dir()
    assert Path(captured["datas"][1][0]).name == "THIRD_PARTY_NOTICES.txt"
    assert Path(captured["datas"][1][0]).is_file()
