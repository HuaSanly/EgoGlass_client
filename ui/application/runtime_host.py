...
from schemas.recording import RecordingLibrary, RecordingState
...
class UnifiedRuntimeHost:
    """Run the recording-only gateway and expose immutable UI snapshots."""
    ...
    def __init__(self, config: RuntimeConfig) -> None:
        ...
        self.imu_telemetry = ImuTelemetryRuntime()
        self.webrtc = WebRtcSessionRuntime(
            self.pairing_token,
            display_frame_sink=self.frame_buffer,
            display_imu_sink=self.imu_telemetry,
        )
        ...
        self._library = RecordingLibrary(recordings=[])
    ...
    async def _close_local_resources(self) -> None:
        ...
        await self.imu_telemetry.close()
    ...
    def recording_library(self) -> RecordingLibrary:
        return self._library

    def recording_media_path(self, recording_id: str) -> Path | None:
        known = {item.recording_id for item in self._library.recordings}
        if recording_id not in known:
            return None
        path = self.recordings_root / recording_id / "video.mp4"
        return path if path.is_file() else None

    def recording_reader(
        self,
        recording_id: str,
    ) -> concurrent.futures.Future[CaptureRecordingReader]:
        return self.submit(self.recording.reader(recording_id))

    def imu_telemetry_samples(self, after_sequence: int) -> tuple[ImuTelemetryPoint, ...]:
        return self.imu_telemetry.samples_after(after_sequence)
    ...
    def _thread_main(self) -> None:
        try:
            ...
            LOGGER.exception("EgoGlass runtime stopped with an error")
        finally:
            self._ready.set()
            self._stopped.set()
    ...
    async def _collect_status(self) -> None:
        ...
        while True:
            try:
                ...
                webrtc, stream_control, imu, recording = await asyncio.gather(
                    self.webrtc.status(),
                ...
                snapshot = RuntimeSnapshot(
                    ...
                    imu=imu,
                    imu_telemetry=self.imu_telemetry.snapshot(now_ns),
                    recording=recording,
                ...
                snapshot = RuntimeSnapshot(
                    ...
                    imu=previous.imu,
                    imu_telemetry=self.imu_telemetry.snapshot(),
