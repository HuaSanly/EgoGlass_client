...
_FIELD_DISPLAY: dict[str, tuple[str, str, str, str, tuple[tuple[str, object], ...]]] = {
    ...
    "temporal_processing.enabled": ("Temporal processing", "Enable temporal cleanup", "Finalize offline results with HumanEgo sequence processing", "", ()),
    "temporal_processing.confidence_threshold": ("Temporal processing", "Confidence threshold", "Discard low-confidence hand observations before interpolation", "", ()),
    "temporal_processing.interpolation_max_gap_frames": ("Temporal processing", "Maximum interpolation gap", "Fill only gaps no longer than this frame count", " frames", ()),
    "temporal_processing.minimum_segment_frames": ("Temporal processing", "Minimum hand segment", "Suppress detected segments shorter than this frame count", " frames", ()),
    "temporal_processing.grasp_smoothing_window_frames": ("Temporal processing", "Grasp smoothing window", "Moving-average window for the grasp state", " frames", ()),
    "temporal_processing.grasp_flicker_max_frames": ("Temporal processing", "Grasp flicker limit", "Suppress short state runs bracketed by the opposite state", " frames", ()),
    "temporal_processing.sg_window_frames": ("Kinematic optimization", "Savitzky-Golay window", "Odd position-smoothing window for world trajectories", " frames", ()),
    "temporal_processing.sg_polyorder": ("Kinematic optimization", "Savitzky-Golay order", "Polynomial order used by position smoothing", "", ()),
    "temporal_processing.orientation_ema_alpha": ("Kinematic optimization", "Orientation EMA alpha", "Temporal weight applied to the newest orientation", "", ()),
    "temporal_processing.minimum_smoothing_frames": ("Kinematic optimization", "Minimum smoothing segment", "Keep shorter valid world segments unchanged", " frames", ()),
    "temporal_processing.smoothing_fill_max_gap_frames": ("Kinematic optimization", "Maximum smoothing fill gap", "Maximum internal gap considered by trajectory smoothing", " frames", ()),
    "temporal_processing.maximum_vio_pose_gap_ms": ("Kinematic optimization", "Maximum VIO match error", "Reject world poses farther from the hand frame timestamp", " ms", ()),
...
_FIELDS = (
    ...
    *_hand_algorithm_fields("offline_hand_tracking", ""),
    _field("offline_hand_tracking", "temporal_processing.enabled", "bool"),
    _field("offline_hand_tracking", "temporal_processing.confidence_threshold", "float", maximum=1, step=0.01, decimals=2),
    _field("offline_hand_tracking", "temporal_processing.interpolation_max_gap_frames", "int", maximum=300),
    _field("offline_hand_tracking", "temporal_processing.minimum_segment_frames", "int", minimum=1, maximum=3000),
    _field("offline_hand_tracking", "temporal_processing.grasp_smoothing_window_frames", "int", minimum=1, maximum=301),
    _field("offline_hand_tracking", "temporal_processing.grasp_flicker_max_frames", "int", maximum=300),
    _field("offline_hand_tracking", "temporal_processing.sg_window_frames", "int", minimum=5, maximum=1001),
    _field("offline_hand_tracking", "temporal_processing.sg_polyorder", "int", minimum=1, maximum=10),
    _field("offline_hand_tracking", "temporal_processing.orientation_ema_alpha", "float", minimum=0.01, maximum=1, step=0.01, decimals=2),
    _field("offline_hand_tracking", "temporal_processing.minimum_smoothing_frames", "int", minimum=2, maximum=3000),
    _field("offline_hand_tracking", "temporal_processing.smoothing_fill_max_gap_frames", "int", maximum=300),
    _field("offline_hand_tracking", "temporal_processing.maximum_vio_pose_gap_ms", "int", minimum=1, maximum=10_000),
